...
export default function CheckoutModal({ isOpen, onClose, packageId, price, dominantType }: CheckoutModalProps) {
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    // SendPulse Integration Config
    const senderHash = import.meta.env.VITE_SENDPULSE_SENDER_HASH;
    const formId = import.meta.env.VITE_SENDPULSE_FORM_ID;
    const actionUrl = import.meta.env.VITE_SENDPULSE_FORM_ACTION_URL || "https://login.sendpulse.com/forms/simple/index";

    if (senderHash && formId) {
      try {
        const bodyParams = new URLSearchParams();
        bodyParams.append("sender", senderHash);
        bodyParams.append("form_id", formId);
        bodyParams.append("email", email);
        bodyParams.append("phone", phone);
        bodyParams.append("name", name);
        if (dominantType) bodyParams.append("dominant_type", dominantType);
        if (packageId) bodyParams.append("package_id", packageId);
        bodyParams.append("price", price.toString());

        // We use mode: "no-cors" to safely send standard HTML Form payload
        // directly to SendPulse without getting blocked by browser CORS restrictions.
        ...
        console.log("Данные успешно отправлены в SendPulse (no-cors).");
      } catch (error) {
        console.error("Ошибка при отправке AJAX в SendPulse:", error);
      }
    } else {
      console.log("SendPulse не настроен. Для интеграции добавьте VITE_SENDPULSE_SENDER_HASH и VITE_SENDPULSE_FORM_ID");
    }

    // Always transition to success view to preserve UI conversion flow
    setTimeout(() => {
